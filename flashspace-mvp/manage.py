"""Privileged maintenance, run locally with the same DATABASE_URL as Render.
Render Free has no service shell. Never expose this script as a web endpoint.
"""
import argparse,getpass,os,sqlite3
from pathlib import Path
from backend.server import Store,hash_password,APIError
from backend.postgres_store import select_store

def main():
    p=argparse.ArgumentParser();s=p.add_subparsers(dest='command',required=True)
    r=s.add_parser('reset-password');r.add_argument('email')
    b=s.add_parser('backup');b.add_argument('destination')
    a=p.parse_args();store=select_store(Store,APIError);store.init()
    if a.command=='reset-password':
        ...
        print('Password reset. All existing sessions for that account revoked.')
    else:
        if getattr(store,'is_postgres',False):
            raise SystemExit('Postgres is in use. Back up with your database provider or pg_dump using a securely configured connection. SQLite backup is intentionally disabled; no file was created.')
        ...
        os.chmod(dest,0o600);print('Local SQLite test backup created. Production uses PostgreSQL.')
