"""Flashspace MVP: WSGI API, SQLite, server-side AI and durable ClickUp sync.
...
class AI:
    def __init__(self): self.key = os.getenv('OPENAI_API_KEY', '')
    def call(self, path, data, raw=False, content_type=None):
        if not self.key: raise APIError(503, 'Interview provider is not configured. Your progress is saved.')
        headers = {'Authorization': 'Bearer ' + self.key}
        if content_type: headers['Content-Type'] = content_type
        return remote('POST', 'https://api.openai.com/v1/' + path, headers, data, raw)
    def structured(self, system, payload, name, schema):
        r = self.call('chat/completions', {
        ...
        try:
            ...
            raise APIError(502, 'The interview provider returned an incomplete response. Please retry.') from None
    ...
    def evaluate(self, role, answers):
        ...
        result.update(score=round(sum(c['score'] for c in criteria)/15*100),model=os.getenv('OPENAI_MODEL','gpt-4o-mini'),rubric_version='flashspace-v1',generated_at=now(),human_review_required=True)
        return result
    def transcribe(self, data, mime):
        mime=mime.split(';')[0].lower()
        ext={'audio/webm':'webm','audio/mp4':'mp4','audio/ogg':'ogg','audio/wav':'wav','audio/mpeg':'mp3'}.get(mime)
        if not ext: raise APIError(400,'Unsupported audio format. Please type your answer instead.')
        boundary='fs'+secrets.token_hex(12)
        body=(f'--{boundary}\r\nContent-Disposition: form-data; name="model"\r\n\r\n{os.getenv("OPENAI_STT_MODEL","gpt-4o-mini-transcribe")}\r\n'
              f'--{boundary}\r\nContent-Disposition: form-data; name="file"; filename="answer.{ext}"\r\nContent-Type: {mime}\r\n\r\n').encode()+data+f'\r\n--{boundary}--\r\n'.encode()
        result=self.call('audio/transcriptions',body,content_type='multipart/form-data; boundary='+boundary)
        value=result.get('text','')
        if not isinstance(value,str) or not 1 <= len(value) <= 6000: raise APIError(422,'No usable transcript received. Try recording again or type your answer.')
        return {'text':value}
    def speech(self, question):
        return self.call('audio/speech',{'model':os.getenv('OPENAI_TTS_MODEL','gpt-4o-mini-tts'),'voice':'coral','input':question,'response_format':'mp3'},raw=True)
...
class ClickUp:
    ...
    def list_id(self, role):
        ...
        with self.store.db() as db: db.execute('INSERT OR REPLACE INTO settings VALUES (?,?)',(key,str(found['id'])))
        return str(found['id'])
    @staticmethod
    def description(a):
        ev=a.get('evaluation')
...
class App:
    def __init__(self,db_path=None,roles=None,ai=None,clickup=None,start_worker=True):
        self.store=Store(db_path or os.getenv('DATABASE_PATH',str(ROOT/'data/flashspace.sqlite3')));self.store.init()
    ...
    def bootstrap_admin(self):
        ...
        if not email: return
        if len(password)<16: raise RuntimeError('ADMIN_PASSWORD must contain at least 16 characters.')
        with self.store.db() as db:
    ...
    def route(self,env,body):
        ...
        if path=='/api/health':
            with self.store.db() as db: db.execute('SELECT 1').fetchone()
        ...
        if path=='/api/applications' and method=='GET':
            with self.store.db() as db: ids=[r['id'] for r in db.execute('SELECT id FROM applications WHERE user_id=? ORDER BY rowid DESC',(u['id'],))]
        ...
        if path=='/api/applications' and method=='POST':
            ...
            with self.lock:
                with self.store.db() as db:
                    ...
                    aid=str(uuid.uuid4());a={'id':aid,'user_id':u['id'],'name':u['name'],'email':u['email'],'role_id':role['id'],'role_title':role['title'],'role_snapshot':role,'portfolio':portfolio,'experience':experience,'created_at':now(),'consent_at':now(),'consent_version':'flashspace-v1','status':'interview','answers':[],'question':f"Tell us about a project or experience relevant to {role['title']}. What did you personally contribute?",'evaluation':None}
        ...
        if path=='/api/admin/applications' and method=='GET':
            ...
            with self.store.db() as db: ids=[r['id'] for r in db.execute('SELECT id FROM applications ORDER BY rowid DESC')]
