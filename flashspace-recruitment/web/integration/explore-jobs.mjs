...
export async function checkExploreJobs(browser){
 ...
 try{
  ...
  // Navigation destination is recorded without running an interview in this test.
  ...
  await page.locator('article').filter({hasText:job.title}).getByRole('button',{name:'View role',exact:false}).click();
