...
try{
 ...
 phase='profile-save';await page.locator('.tr-main>header').getByRole('link',{name:'My Profile',exact:true}).click();await page.getByRole('button',{name:'Edit Professional summary',exact:true}).click();await page.getByLabel('Professional summary',{exact:true}).fill('Synthetic browser integration profile.');await page.getByRole('button',{name:'Save profile',exact:true}).click();await page.getByRole('status').filter({hasText:'Profile saved.'}).waitFor();
 phase='profile-reload';await page.reload();await page.getByRole('button',{name:'Edit Professional summary',exact:true}).click();assert.equal(await page.getByLabel('Professional summary',{exact:true}).inputValue(),'Synthetic browser integration profile.');await page.getByRole('button',{name:'Cancel',exact:true}).click();
