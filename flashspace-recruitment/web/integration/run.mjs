...
try{
 ...
 await page.goto(origin+'/candidate/signup');
 ...
 await page.locator('.tr-main>header').getByRole('link',{name:'My Profile',exact:true}).click();await page.getByLabel('Professional summary',{exact:true}).fill('Synthetic browser integration profile.');await page.getByRole('button',{name:'Save profile',exact:true}).click();await page.getByRole('status').filter({hasText:'Profile saved.'}).waitFor();await page.reload();assert.equal(await page.getByLabel('Professional summary',{exact:true}).inputValue(),'Synthetic browser integration profile.');
 await page.goto(origin+'/candidate/workspace/jobs');await page.getByRole('button',{name:'View role',exact:false}).first().click();await page.getByRole('button',{name:'Apply for this role',exact:true}).click();await page.getByLabel('Relevant experience',{exact:true}).fill('Synthetic browser test with relevant project experience.');await page.locator('#consent').click();await page.getByRole('button',{name:'Apply & start interview',exact:true}).click();await page.waitForURL('**/interview-v2?application=*');
 const aid=new URL(page.url()).searchParams.get('application');
 await page.getByRole('button',{name:'Begin interview',exact:true}).waitFor();assert(await page.getByRole('button',{name:'Begin interview',exact:true}).isDisabled());
 ...
 const recruiter=await browser.newContext({viewport:{width:1440,height:1000}}),review=await recruiter.newPage();review.setDefaultTimeout(20000);await review.goto(origin+'/recruiter/login');await review.getByLabel('Work email',{exact:true}).fill('team@stirringminds.com');await review.getByLabel('Password',{exact:true}).fill('synthetic-browser-password-123');await review.getByRole('button',{name:'Sign in',exact:true}).click();await review.waitForURL('**/recruiter/workspace/dashboard');
