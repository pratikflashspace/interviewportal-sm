...
// Keep conservative state across delayed/duplicate speech events.
export class TurnController {
 constructor(clock=()=>performance.now()){
  this.clock=clock;this.epoch=0;this.state='paused';this.segments=new Map();
  this.pending=new Set();this.activeSpeech=new Set();this.finalized=new Set();
 ...
 begin(){
  ...
  this.activeSpeech.clear();this.finalized.clear();this.speaking=false;
 ...
 partial(index){
  if(this.inactive()||!this.valid(index)||this.finalized.has(index)||this.segments.has(index))return false;
  // A partial can arrive without a preceding speech-start event. Treat it as
  // active speech, not just text, to block explicit Finish and TTS playback.
  this.epoch++;this.pending.add(index);this.activeSpeech.add(index);
  this.speaking=true;this.lastSpeech=this.clock();this.state='candidate-speaking';return true;
 }
 speechEnd(index){
  if(this.inactive())return;
  // Legacy callers may omit index. Indexed events avoid an old utterance's
  // late end clearing a newer utterance that is still being spoken.
  if(index===undefined)this.activeSpeech.clear();
  else {if(!this.valid(index))return;this.activeSpeech.delete(index);}
 ...
 final(index,text){
  ...
  // Final text is not evidence of acoustic silence. Wait for speech-end too.
 }
