// Flashspace v2 pilot: existing paper/lilac visual language, prominent honest
// listening state; no per-question recording-start control or simulated voice.
...
export default function Conversation(){
 ...
 const [status,setStatus]=useState('paused'),[error,setError]=useState(''),[draft,setDraft]=useState(''),[busy,setBusy]=useState(false),[typing,setTyping]=useState(false),[bankData,setBankData]=useState(null),[reports,setReports]=useState([]);
 const live=useRef(null),media=useRef(null),sound=useRef(null),ctl=useRef(new TurnController()),generation=useRef(0),checking=useRef(false),request=useRef(null),paused=useRef(true),timer=useRef(null),retryAfter=useRef(0),introDone=useRef(false),closing=useRef(Promise.resolve());
 ...
 function stopAudio(){if(sound.current){const s=sound.current;sound.current=null;s.audio.pause();URL.revokeObjectURL(s.url);s.resolve(false);}}
 function closeMedia(){
  const m=media.current;media.current=null;if(!m)return closing.current;
  m.socket.onmessage=null;m.socket.onerror=null;m.socket.onclose=null;
 ...
 async function load(aid){halt();setError('');try{const f=await api('/v2/applications/'+aid);live.current=f;setFlow(f);setDraft('');introDone.current=f.answers.length>0;}catch(e){setError(e.message);}}
 async function create(e){e.preventDefault();setBusy(true);setError('');try{const body=Object.fromEntries(new FormData(e.currentTarget));const f=await api('/v2/applications',{...body,consent:true,consent_version:consent});live.current=f;setFlow(f);introDone.current=f.answers.length>0;setApps(await api('/applications'));}catch(e){setError(e.message);}finally{setBusy(false);}}
 async function openMic(){
  ...
  try{
   ...
   node.port.onmessage=e=>{if(!paused.current&&gen===generation.current&&socket.readyState===1){if(socket.bufferedAmount>64000){halt();setError('Network is too slow. Listening paused; unsaved speech may need repeating.');}else socket.send(e.data);}};
   await new Promise((resolve,reject)=>{const timeout=setTimeout(()=>reject(new Error('Voice connection timed out.')),20000);
    socket.onmessage=e=>{let msg;try{msg=JSON.parse(e.data);}catch{return;}
     ...
     if(msg.event==='vad.speech_start'){
      ctl.current.speechStart(msg.utterance_idx);stopAudio();
      if(request.current){halt();setError('Speech resumed while an answer was saving. Reload saved state before continuing; do not resubmit blindly.');return;}
     }
    ...
    socket.onclose=()=>{clearTimeout(timeout);reject(new Error('Voice connection closed.'));if(!paused.current&&gen===generation.current){halt();setError('Connection lost. Saved answers remain; reload and reconnect to continue.');}};
   });
 ...
 async function speak(questionId,intro=false){
  ...
  const blob=await api('/v2/applications/'+live.current.application_id+(intro?'/intro':'/speech'),intro?{}:{question_id:questionId},true);
  ...
  return new Promise(resolve=>{
   ...
   audio.onerror=()=>{halt();setError('Audio playback failed. Use typing or reconnect.');resolve(false);};
   audio.play().catch(()=>{halt();setError('Browser blocked playback. Use Begin / reconnect to grant audio permission, or type.');resolve(false);});
  });
 }
 async function begin(){
  ...
  try{
   ...
   if(!introDone.current){const finished=await speak(null,true);if(!finished){halt();setError('Introduction interrupted. Reconnect when ready; no answer was submitted.');return;}introDone.current=true;await closeMedia();ctl.current.begin();await openMic();}
 ...
 async function submit(text,token=null){
  if(request.current||!text.trim())return;const f=live.current,gen=generation.current;
  ...
  try{
   ...
   live.current=next;setFlow(next);request.current=null;
   if(next.status==='completed'){halt();ctl.current.complete();setStatus('completed');setDraft('');return;}
   if(paused.current||gen!==generation.current||(token!==null&&!ctl.current.canPlay(token))){setDraft('');return;}
   await closeMedia();ctl.current.begin();await openMic();setDraft('');await speak(next.active.id);
