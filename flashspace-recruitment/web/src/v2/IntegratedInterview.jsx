...
export default function IntegratedInterview(){
 const [user,setUser]=useState(undefined),[flow,setFlow]=useState(null),[consent,setConsent]=useState(false),[status,setStatus]=useState('ready'),[error,setError]=useState(''),[text,setText]=useState(''),[voiceState,setVoiceState]=useState('idle'),[busy,setBusy]=useState(false),[playback,setPlayback]=useState(null),[draftStatus,setDraftStatus]=useState(''),[disclosure,setDisclosure]=useState({qid:null,count:0}),[started,setStarted]=useState(false),[help,setHelp]=useState(false),[devicesReady,setDevicesReady]=useState(false),[micDetected,setMicDetected]=useState(false),[heard,setHeard]=useState(false);
 ...
 async function checkDevices(){setBusy(true);setError('');setDevicesReady(false);setMicDetected(false);setHeard(false);clearInterval(deviceTimer.current);try{const stream=await preflight.current.open();if(!mounted.current)return;setDevicesReady(true);deviceTimer.current=setInterval(()=>{if(preflight.current.level()>.015)setMicDetected(true);},100);stream.getTracks().forEach(t=>t.onended=()=>{setDevicesReady(false);setError('The microphone disconnected. Run device checks again.');});}catch(e){if(mounted.current)setError(e.message);}finally{if(mounted.current)setBusy(false);}}
 ...
 async function openSpeech(){
  ...
  await new Promise((resolve,reject)=>{const t=setTimeout(()=>reject(retryable(Error('Voice connection timed out.'))),20000);state.cancel=()=>{clearTimeout(t);reject(Error('Listening cancelled.'));};
   ...
   socket.onclose=()=>{clearTimeout(t);if(!live){reject(retryable(Error('Voice connection failed.')));return;}if(!paused.current&&run===epoch.current)stopWithError('Voice connection lost. The interview is paused.');};
  });
 ...
 async function begin(){
  if(startGuard.current||saving.current||started||!consent||!devicesReady||!micDetected||!heard)return;
 ...
 {!started&&!done&&<section className="room-preflight"><h2>Before you begin</h2><p>This is a voice-only conversation. The interviewer asks one question at a time, speaks it aloud, and listens automatically — you never press a record button. The device check measures your microphone locally; it does not record, upload or contact the AI provider. No camera is used.</p><p>Your speech is transcribed and processed by Sarvam for this interview. Drafts and submitted answers are saved, and submitted transcripts and reports are sent to ClickUp for authorised human review. External provider retention is not verified as zero.</p><button onClick={checkDevices} disabled={busy}>Check your microphone</button><p role="status">{devicesReady?(micDetected?'Microphone connected · signal detected':'Microphone connected · speak to test'):'Device checks not completed'}</p>{devicesReady&&<><button onClick={playTest}>Play test sound</button><label><input type="checkbox" checked={heard} onChange={e=>setHeard(e.target.checked)}/>I heard the test sound.</label></>}
 <label><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/>I consent to voice processing as described.</label>
 <button onClick={begin} disabled={busy||!devicesReady||!micDetected||!heard||!consent}>Begin interview</button></section>}
