...
export async function jobsRequest(path,{body,signal,fetcher=globalThis.fetch,timeoutMs=path==='/logout'?65000:12000}={}){
 ...
 try{
  ...
  if(!r.ok){const e=Error(r.status===401?'Your session has expired. Sign in again.':r.status===403?'Candidate access is required.':r.status===409?(typeof data.error==='string'?data.error:'This role or application changed. Refresh jobs or open My Applications.'):r.status===429?'Too many attempts. Please wait before trying again.':r.status===400?(typeof data.error==='string'?data.error:'Check your application details.'):'Could not load or save this request. Please try again.');e.status=r.status;throw e;}
  return data;
