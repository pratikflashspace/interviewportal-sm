...
import {publicRoles,filterOptions,filterValue,matches,display,existingDestination,jobsRequest} from './explore-jobs.mjs';
...
test('all six filters operate on exact saved values',()=>{for(const [key,value] of [['department','IT'],['experience','0-1'],['location',source.location],['work_mode','Hybrid'],['type','Full Time'],['skills','Python']])assert.deepEqual(matches(rows(),'',{[key]:value}).map(r=>r.id),['one']);assert.equal(matches(rows(),'',{department:'IT',skills:'CRM'}).length,0);assert.equal(matches(rows(),'',{work_mode:'Remote'}).length,0);});
