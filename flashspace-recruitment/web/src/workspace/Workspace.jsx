...
function Auth({role,signup}){
 ...
 async function submit(e){e.preventDefault();if(busy)return;setBusy(true);setError('');const data=Object.fromEntries(new FormData(e.currentTarget));if(signup){const phone=String(data.phone||'').replace(/\s+/g,'');if(!/^\d{10}$/.test(phone)){setError('Phone number must be exactly 10 digits.');setBusy(false);return;}data.phone=phone;}try{await request('/auth/'+role+'/'+(signup?'signup':'login'),data);go('/'+role+'/workspace/dashboard');}catch(e){setError(e.message);}finally{setBusy(false);}}
 return <main className="tr-auth"><Brand/><a href="/account-type">← Choose account type</a><section><p className="tr-eyebrow">{role==='candidate'?'YOUR NEXT CHAPTER':'STIRRING MINDS · AUTHORISED ACCESS'}</p><h1>{signup?'Create your candidate account':'Welcome back'}</h1><p>{role==='candidate'?'Sign in to continue your job search.':'Sign in to manage your hiring workspace.'}</p><form onSubmit={submit}>{signup&&<label>Full name<input name="name" required minLength={2} maxLength={100} autoComplete="name"/></label>}<label>{role==='recruiter'?'Work email':'Email address'}<input name="email" type="email" required autoComplete="email"/></label><label>Password<PasswordField name="password" autoComplete={signup?'new-password':'current-password'}/></label>{signup&&<><label>Confirm password<PasswordField name="confirm_password" autoComplete="new-password"/></label><label>Phone number<input name="phone" type="tel" inputMode="numeric" pattern="\d{10}" required maxLength={10} autoComplete="tel" placeholder="10-digit mobile number"/></label></>}{error&&<p role="alert">{error}</p>}<button disabled={busy}>{busy?'Please wait…':signup?'Create candidate account':'Sign in'}</button></form>{role==='candidate'&&<a href={signup?'/candidate/login':'/candidate/signup'}>{signup?'Already have an account? Sign in':'Create a candidate account'}</a>}{role==='recruiter'&&<p className="tr-note">Access is restricted to the authorised Stirring Minds recruiter. No public signup.</p>}</section></main>;
}
