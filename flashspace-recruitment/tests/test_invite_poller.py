...
class FakeCU:
    """ClickUp fake with folders/lists/tasks/fields/comments and dropdown semantics."""
    ...
    def call(self, method, path, data=None):
        ...
        if method == 'GET' and path.startswith('list/') and path.endswith('/field'):
            lid = path.split('/')[1]
            return {'fields': list(self.fields.get(lid, {}).values())}
        if method == 'POST' and '/field' in path and '/task/' not in path:
            lid = path.split('/')[1]
            fid = f'fld-{lid}-{len(self.fields.get(lid, {}))}'
            field = {'id': fid, 'name': data['name'], 'type': data['type'], 'type_config': data['type_config']}
            field['type_config'] = {'options': [{'id': 'yes-id', 'name': 'Yes', 'orderindex': 0},
                                                 {'id': 'no-id', 'name': 'No', 'orderindex': 1}]}
            self.fields.setdefault(lid, {})[fid] = field
            return {'field': field}
        if method == 'GET' and '/task?' in path or path.startswith('list/') and path.endswith('task?include_closed=true&page=0&subtasks=false'):
            lid = path.split('/')[1]
            out = []
...
class InvitePollerTests(unittest.TestCase):
    def setUp(self):
    ...
    def run_cycle(self):
        ...
        actions = ip.run_once(self.cu, self.cw, state, dry=False)
        ip.save_state(self.state_path, state)
        return actions

    def set_invite(self, tid, value):
        # ensure the field exists first, mimicking ensure_invite_field
        state = ip.load_state(self.state_path)
    ...
    def test_creates_dropdown_and_sends_for_interview_yes(self):
        ...
        self.assertIn('Sign in and make your candidate profile', text)
        self.assertTrue(actions and actions[0].startswith('SENT'))
        # success comment recorded on the task
    ...
    def test_profile_yes_invites_all_roles(self):
        ...
        actions = self.run_cycle()
        self.assertEqual(len(self.cw.sent), 2)  # Sales + Operations
    ...
    def test_chatwoot_failure_comments_once_not_spam(self):
        ...
        a2 = self.run_cycle()
        self.assertEqual(self.cw.sent, [])
        self.assertEqual(len(self.cu.comments['i1']), 1)  # not duplicated
        self.assertTrue(any('FAILED' in a for a in a1))
        self.assertEqual(a2, [])  # same failure text is not re-commented

    def test_unset_yes_tracks_reset(self):
        self.set_invite('i1', 'yes-id')
        self.run_cycle()
        self.set_invite('i1', None)
        actions_mid = self.run_cycle()
        self.assertEqual(actions_mid, [])
        self.set_invite('i1', 'yes-id')
        self.run_cycle()
        self.assertEqual(len(self.cw.sent), 2)
    ...
    def test_dry_run_sends_nothing(self):
        self.set_invite('i1', 'yes-id')
        state = ip.load_state(self.state_path)
        actions = ip.run_once(self.cu, self.cw, state, dry=True)
        ip.save_state(self.state_path, state)
        self.assertEqual(self.cw.sent, [])
    ...
    def test_state_file_roundtrip(self):
        ...
        ip.save_state(self.state_path, state)
        self.assertEqual(ip.load_state(self.state_path), state)
